...
// NOTE: This component now fetches its own data client-side
// to avoid passing data from the parent Server Component.
async function fetchTopPages(): Promise<AnalyticsData> {
  // Use a relative path to fetch from the internal API route
  const res = await fetch('/api/ga/summary', { cache: 'no-store' }); // Use no-store for dynamic client-side fetch
  ...
  // Ensure the topPages property exists, defaulting to an empty array
    ...
    if (!data || chartData.length === 0) {
        return (
            <Alert>
                ...
                <AlertDescription>Aún no hay suficientes datos de páginas vistas para mostrar en este gráfico.</AlertDescription>
            </Alert>
        );
    }
